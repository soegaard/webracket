...
so avoid using the `--stdlib` flag when compiling this file.

If the current directory is `test/` then this command will
make the `webracket` command, run the tests, and format them.

raco make ../webracket.rkt && racket -l errortrace -t ../webracket.rkt -- -r test-basics.rkt | racket ../tools/pretty.rkt
