...
(list
 (list "Canvas and DOMRect wrappers"
       (let ()
         ...
         (canvas-to-blob canvas
                         ...
                         "image/png"
                         0.5)
         (check-equal (vector-length (js-ref (html-canvas-element-raw canvas) "calls")) 5 "canvas call count")
         (define get-context-call (vector-ref (js-ref (html-canvas-element-raw canvas) "calls") 0))
         ...
         (check-true (canvas-rendering-context-2d? ctx) "canvas context wrapper")
         (check-equal (js-ref (canvas-rendering-context-2d-raw ctx) "direction") "ltr" "canvas context direction")
         ...
         (define fill-call (vector-ref (js-ref (canvas-rendering-context-2d-raw ctx) "calls") 0))
         ...
         (define (canvas-call-recorded? name)
           (let ([calls (js-ref (canvas-rendering-context-2d-raw ctx) "calls")])
             (let loop ([i 0])
               (cond
         ...
         (check-true (image-data? image-data) "canvas image data wrapper")
         ...
         (check-true (image-data? copied-image-data) "canvas image data copy wrapper")
         ...
         (check-true (image-data? image-data-from-canvas) "canvas get image data wrapper")
         ...
         (check-true (dom-matrix? matrix) "canvas transform wrapper")
         ...
         (check-true (text-metrics? measure-text) "canvas measure text wrapper")
 ...
 (list "Media and image wrappers"
       (let ()
         ...
         (check-true (media-source? src-object) "media src object")
         (check-equal (js-ref (media-source-raw src-object) "kind") "source" "media src object raw")
