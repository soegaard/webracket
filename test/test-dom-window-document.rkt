...
(list "Document and element wrappers"
      (let ()
         ...
         (check-equal (element-class-name leaf) "leaf" "element class name")
         (element-set-class-name! leaf 'sprout)
         (check-equal (element-class-name leaf) "sprout" "element set class name")
         ...
         (check-equal (js-ref (document-query-selector-all 'span) "length") 1 "document query selector all")
