...
(list
 ...
 (list "WebSocket wrappers"
       (let* ([_  (js-eval
         ...
         (js-send/extern/nullish (websocket-raw ws) "dispatch" (vector "open" (js-eval "({type: 'open'})")))
         (js-send/extern/nullish (websocket-raw ws) "dispatch" (vector "message" (js-eval "({type: 'message', data: 'ping'})")))
         (js-send/extern/nullish (websocket-raw ws) "dispatch" (vector "error" (js-eval "({type: 'error'})")))
         (websocket-remove-event-listener! ws "message" listener #t)
         (js-send/extern/nullish (websocket-raw ws) "dispatch" (vector "close" (js-eval "({type: 'close'})")))
         ...
         (check-equal (js-ref (websocket-raw ws) "protocols") (vector "chat" "v2") "wrapper protocols")
         (check-equal (js-ref (websocket-raw ws) "sent") (vector "hello") "wrapper send")
         (check-equal (js-ref (websocket-raw ws) "closed") (vector (vector 1000 "done")) "wrapper close")
         (check-equal (js-ref (websocket-raw ws) "lastAddOptions") #t "wrapper add listener options")
         (check-equal (js-ref (websocket-raw ws) "lastRemoveOptions") #t "wrapper remove listener options")
