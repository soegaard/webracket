...
(list
 ...
 (list "Iterator static helpers"
       (let ()
         (check-true (iterable? (js-array/extern (vector 1 2 3)))
                     "array is iterable")
         (check-true (iterable? "abc")
                     "string is iterable")
         (check-true (iterable? (fresh-iter))
                     "wrapped iterator is iterable")
         (check-true (not (iterable? 42))
                     "number is not iterable")
         (define zip-options (make-iterator-zip-options 'shortest #f))
         (check-true (iterator-zip-options? zip-options)
                     "iterator zip options struct")
         ...
         (check-equal (iterator->vector concatenated)
                      #(1 2 3 4)
                      "iterator concat")
 ...
 (list "Iterator instance methods"
       (let ()
         ...
         (check-equal (iterator->vector (iterator-map (fresh-iter)
                                                       (lambda (n) (* n 2))))
                      #(2 4 6 8)
                      "iterator map")
         (check-equal (iterator->vector (iterator-filter (fresh-iter)
                                                          (lambda (n) (even? n))))
                      #(2 4)
                      "iterator filter")
         (check-equal (iterator->vector (iterator-flat-map (fresh-iter)
                                                            (lambda (n)
                                                              (js-array/extern (vector n n)))))
                      #(1 1 2 2 3 3 4 4)
                      "iterator flatMap")
         (check-equal (iterator->vector (iterator-drop (fresh-iter) 2))
                      #(3 4)
                      "iterator drop")
         (check-equal (iterator->vector (iterator-take (fresh-iter) 2))
                      #(1 2)
                      "iterator take")
         ...
         (check-equal (iterator-reduce (fresh-iter)
                                       (lambda (acc n) (+ acc n)))
                      10
                      "iterator reduce omitted initial")
         (check-equal (iterator-reduce (iterator-from (js-array/extern (vector #f 2 3)))
                                       (lambda (acc n) (if acc (+ acc n) n))
                                       (lambda () #f))
                      5
                      "iterator reduce thunk initial")
         (define seen (box 0))
