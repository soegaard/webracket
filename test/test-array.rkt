...
(list
 (list "Array wrappers"
       (let ()
         ...
         (check-true (array-is-array? arr) "array predicate")
         (define source #(7 8 9))
         ...
         (check-equal (array->vector (array-from source)) #(7 8 9) "array from")
         (check-equal (array->vector (vector->array #(4 5))) #(4 5) "vector to array")
         (check-equal (array->vector (list->array '(6 7))) #(6 7) "list to array")
         (check-equal (array->vector (sequence->array '(8 9))) #(8 9) "sequence to array list")
         (check-equal (array->vector (sequence->array #(10 11))) #(10 11) "sequence to array vector")
         (check-equal (array-join arr "-") "1-2-3" "array join")
         (check-equal (array->vector (array-slice arr 1 #f)) #(2 3) "array slice")
         (check-equal (array->vector (array-concat (array-of 1 2) (array-of 3 4)))
                      #(1 2 3 4)
                      "array concat")
         ...
         (check-equal (array->vector (array-copy-within! (array-of 1 2 3 4) 1 2))
                      #(1 3 4 4)
                      "array copy within")
         (check-equal (array->vector (array-fill! (array-of 1 2 3 4) 9 1 3))
                      #(1 9 9 4)
                      "array fill")
         (check-equal (array->vector (array-reverse! (array-of 1 2 3))) #(3 2 1) "array reverse")
         (check-equal (array->vector (array-sort! (array-of 3 1 2))) #(1 2 3) "array sort")
         (check-equal (array->vector (array-splice! (array-of 1 2 3 4) 1 2 9 8))
                      #(2 3)
                      "array splice removed")
         (check-equal (array->vector (array-to-reversed (array-of 1 2 3)))
                      #(3 2 1)
                      "array to reversed")
         (check-equal (array->vector (array-to-sorted (array-of 3 1 2))) #(1 2 3) "array to sorted")
         (check-equal (array->vector (array-to-spliced (array-of 1 2 3 4) 1 2 9 8))
                      #(1 9 8 4)
                      "array to spliced")
         (check-equal (array->vector (array-with (array-of 1 2 3) 1 99)) #(1 99 3) "array with")
