   ...
   '(div (@ (id "query-root"))
         ...
         (div (@ (id "query-host"))
              ...
              (div (@ (class "card") (id "card-2"))
                   ...
                   (span (@ (class "tag")) "Two"))
              (input (@ (id "value-host") (value "initial")))
              (div (@ (id "data-host") (data-state "cold")))
              (div (@ (id "tree"))
                   (section (@ (id "branch-a"))
                            (p (@ (class "leaf")) "Leaf A")
                            (p (@ (class "leaf")) "Leaf B"))
...
(define tree ($ "#tree"))
(define sel ($ "#hw"))
(define toggle-sel ($ "#toggle-host"))
(define card-sel ($ ".card"))
(define leaf-sel ($ ".leaf"))
(define text-sel ($ "#text-host"))
(define value-sel ($ "#value-host"))
(define data-sel ($ "#data-host"))
(define delegate-sel ($ "#delegation-host"))
(define trigger-sel ($ "#delegated-button"))
(define click-count 0)
(define once-count 0)
(define trigger-count 0)
(define trigger-delegate-count 0)
...
(define (handle-once _evt)
  (set! once-count (add1 once-count)))

(define (handle-trigger _evt)
  (set! trigger-count (add1 trigger-count)))

(define (handle-trigger-delegate matched _evt)
  (set! trigger-delegate-count (add1 trigger-delegate-count))
...
(list
 (list "query include-lib"
       (let ()
         ...
         (check-equal ($text! text-sel "Text node updated") text-sel "selection text! returns selection")
         (check-equal (.text! text-sel "Text node updated again") text-sel "chainable text! returns selection")
         (check-equal (.text text-sel) "Text node updated again" "selection text! applied")
         (check-true ($has-class? toggle-sel "alpha") "toggle class initial")
         (check-equal ($toggle-class! toggle-sel "alpha") toggle-sel "selection toggle-class! returns selection")
         (check-equal ($has-class? toggle-sel "alpha") #f "toggle class removed")
         (check-equal (.toggle-class! toggle-sel "alpha") toggle-sel "chainable toggle-class! returns selection")
         (check-equal ($has-class? toggle-sel "alpha") #t "toggle class restored")
         (check-equal ($val value-sel) "initial" "selection val")
         (check-equal (.val value-sel) "initial" "chainable val")
         (check-equal ($val! value-sel "updated") value-sel "selection val! returns selection")
         (check-equal (.val! value-sel "updated-again") value-sel "chainable val! returns selection")
         (check-equal ($val value-sel) "updated-again" "selection val! applied")
         (check-equal ($data data-sel "state") "cold" "selection data")
         (check-equal (.data data-sel "state") "cold" "chainable data")
         (check-equal ($data! data-sel "state" "warm") data-sel "selection data! returns selection")
         (check-equal (.data! data-sel "state" "hot") data-sel "chainable data! returns selection")
         (check-equal ($data data-sel 'state) "hot" "selection data! applied")
         ...
         (check-equal ($once "click" handle-once sel) sel "selection once returns selection")
         (check-equal (.once sel "click" handle-once) sel "chainable once returns selection")
         (check-equal once-count 0 "once count before dispatch")
         (check-true (js-eval "document.getElementById('hw').dispatchEvent(new Event('click', { bubbles: true }))")
                     "dispatch once click event")
         (check-equal once-count 1 "once count after first dispatch")
         (check-true (js-eval "document.getElementById('hw').dispatchEvent(new Event('click', { bubbles: true }))")
                     "dispatch once click event after remove")
         (check-equal once-count 1 "once count after second dispatch")
         ...
         (check-equal ($on "click" handle-trigger trigger-sel) trigger-sel "selection trigger on returns selection")
         (check-equal (.on trigger-sel "click" handle-trigger) trigger-sel "chainable trigger on returns selection")
         (check-equal ($on-delegate "click" "button" handle-trigger-delegate delegate-sel)
                      delegate-sel
                      "selection trigger delegate returns selection")
         (check-equal (.on-delegate delegate-sel "click" "button" handle-trigger-delegate)
                      delegate-sel
                      "chainable trigger delegate returns selection")
         (check-equal trigger-count 0 "trigger count before dispatch")
         (check-equal trigger-delegate-count 0 "trigger delegate count before dispatch")
         (check-equal ($trigger "click" trigger-sel) trigger-sel "selection trigger returns selection")
         (check-equal (.trigger trigger-sel "click") trigger-sel "chainable trigger returns selection")
         (check-equal trigger-count 2 "trigger count after dispatch")
         (check-equal trigger-delegate-count 2 "trigger delegate count after dispatch")
