...
;; Purpose: Return a sorted list of symbols using symbol<? ordering.
(define (sort-symbols syms)
  ;; Purpose: Insert a symbol into a sorted list.
...
;; Purpose: Convert a symbol into a title string for headings.
(define (symbol->title s)
  (symbol->string s))
...
;; Purpose: Convert a 0-1 progress ratio into a rounded percentage.
(define (format-percent pct)
  (inexact->exact (round (* 100 pct))))

;; Purpose: Build the Racket documentation URL for a primitive.
(define (primitive-url sym)
  (string-append "https://docs.racket-lang.org/search/index.html?q="
                 (symbol->string sym)))

;; Purpose: Classify a primitive by implementation status.
...
;; Purpose: Render a primitive list item with status badge and link.
...
;; Purpose: Create a stable HTML id from a section title.
(define (section-id title)
  ;; Build a slug without regex: ASCII alphanumerics separated by single hyphens.
  ;; Purpose: Recognize ASCII lowercase letters and digits.
...
;; Purpose: Compute summary stats for a section.
...
;; Purpose: Render a status card for a section of primitives.
...
;; Purpose: Render a chapter block containing section cards.
...
;; Purpose: Return the first list element that satisfies pred.
...
;; Purpose: Build attention metadata entries that match known sections.
...
;; Purpose: Take up to n items from a list.
(define (take-up-to lst n)
  (if (or (zero? n) (null? lst))
      '()
      (cons (car lst) (take-up-to (cdr lst) (sub1 n)))))

;; Purpose: Select top attention items with low completion.
...
;; Purpose: Render an attention card summary for a section.
...
;; Purpose: Render the legend explaining status chips.
...
;; Purpose: Render a callout highlighting strongest and weakest sections.
...
;; Purpose: Convert a DOM NodeList into a Racket list.
...
;; Purpose: Store event handlers so they stay reachable.
(define (remember-status-handler! handler)
  (set! status-handler-store (cons handler status-handler-store))
  (void))

;; Purpose: Check whether an element's classList contains a class.
...
;; Purpose: Extract trimmed textContent from an element.
...
;; Purpose: Expand and scroll to a status section by id.
...
;; Purpose: Wire smooth scrolling for attention card links.
...
;; Purpose: Derive the status label from a primitive row element.
...
;; Purpose: Resolve the display name for a row for sorting.
...
;; Purpose: Return status precedence ordering for a group filter.
...
;; Purpose: Find the index of a status in an ordering list.
...
;; Purpose: Sort a status list in place by group and name.
(define (status-sort-list! list-el active-group active-dir)
  ...
  ;; Purpose: Compare two decorated rows for sorting order.
  (define (item<? a b)
...
;; Purpose: Sync filter button states to active group.
...
;; Purpose: Initialize filter buttons and sorting per section.
(define (init-status-filter!)
  ...
  (for ([section (in-list sections)])
    ...
    (when (and list-el (pair? button-list))
      ...
      ;; Purpose: Refresh button state and re-sort the list.
...
;; Purpose: Attach handlers to collapse expanded sections.
...
;; Purpose: Initialize all behavior for the status page.
...
;; Purpose: Render the implementation status dashboard page.
