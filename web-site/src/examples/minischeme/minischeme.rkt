      ...
      (let loop ([mode        'eval]
        ...
        (define (apply-now proc args env kont)
          (define (apply-value proc args)
            ...
            (eval-now (cons f-id arg-ids) tmp-env))
          (if (prim? proc)
              (let ([pname (prim-name proc)])
                (cond
                  ...
                  [(eq? pname 'map)
                   ...
                   (ensure-list 'map lst)
                   (continue 'value
                             (map (λ (x) (apply-value f (list x))) lst)
                             env
                             kont)]
                  [(eq? pname 'for-each)
                   ...
                   (for-each (λ (x) (apply-value f (list x))) lst)
                  ...
                  [(eq? pname 'filter)
                   ...
                   (let loop-filter ([xs lst] [acc '()])
                     (if (null? xs)
                         (continue 'value (reverse acc) env kont)
                         (let ([ok (apply-value pred (list (car xs)))])
                           (loop-filter (cdr xs)
                                        (if (false? ok)
                                            acc
                                            (cons (car xs) acc))))))]
