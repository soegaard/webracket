...
(define (render dc h+top l dx dy) (error 'render "todo")) ; todo
  ...
  (λ args
    (match args
      ...
      [(list 'image-smoothing-enabled val)
       (js-set-canvas2d-image-smoothing-enabled! (to-boolean val))]
      ; image-smooting-quality  -- currently missing from Firefox
      [(list 'letter-spacing val)
       (js-set-canvas2d-letter-spacing! (to-string val))]
      [(list 'line-cap val)
       (js-set-canvas2d-line-cap! (to-string val))]
      [(list 'line-dash-offset val)
       (js-set-canvas2d-line-dash-offset! (to-real val))]
      <todo-insert-remaining-here>)))
...
(let ()
  ...
  (js-set-canvas2d-font!       ctx "64px 'Fira Sans'")
  #;(js-canvas2d-stroke-text     ctx "Hello World" 400. 200. (void)) ; outline
  ; the fill-style is used for both paths and text
  (js-canvas2d-fill-text     ctx "Hello World" 400. 200. (void))
