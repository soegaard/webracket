...
is rejected. Local pax and GNU long-name extension records must have a following
...
downloaded or file-backed gzip data by `.tar.gz`/`.tgz` source names or by
gzip magic bytes. Decompression failures are reported as WebRacket VFS gzip
errors.
...
permissions `#o777`, and a stable positive identity derived from the synthetic
...
VFS roots, including synthetic mount parents. If a backend is explicitly
mounted at `/`, `filesystem-root-list` reports `/`.
