...
;; Adapted from racket/src/cs/rumble/patricia.ss
...
(define-values
  ...
  (let ()
    ...
    (define key-equal-hash-code equal-hash-code)
    (define key-equal?          equal?)
    ...
    (struct intmap (eqtype [root #:mutable])
      #:extra-constructor-name make-intmap)

    (struct Br (count prefix mask left right)  #:extra-constructor-name make-Br)
    (struct Lf (hash key value)                #:extra-constructor-name make-Lf)
    (struct Co (hash pairs)                    #:extra-constructor-name make-Co)
    ...
    (define (intmap-shell-sync! dst src)
      (set-intmap-root! dst (intmap-root src)))
    ...
    (define-syntax-rule (highest-set-bit x1)
      (let* ([x2 (fxior x1 (fxrshift/logical x1 1))]
             [x3 (fxior x2 (fxrshift/logical x2 2))]
             [x4 (fxior x3 (fxrshift/logical x3 4))]
             [x5 (fxior x4 (fxrshift/logical x4 8))]
             [x6 (fxior x5 (fxrshift/logical x5 16))]
             [x7 x6 ; webracket has 30-bit fixnums
                 #;(if (> (fixnum-width) 32)
                       (fxior x6 (fxrshift/logical x6 32))
                       x6)])
        (fxxor x7 (fxrshift/logical x7 1))))
    ...
    (define (intmap-map t proc)
      (reverse (intmap-fold t '() (lambda (k v xs) (cons (proc k v) xs)))))
