...
async function main() {
  ...
  for (const [name, generatedSelector, referenceSelector] of checks) {
    try {
      ...
      computed.push({ name, generatedSelector, referenceSelector, error: String(e) });
      await page.goto(generatedUrl, { waitUntil: "networkidle" });
    }
  }
  ...
  const errorCount = computed.filter((row) => row.error).length;
  ...
  const byName = new Map(computed.filter((r) => !r.error).map((r) => [r.name, r]));
  const gapPairs = [
