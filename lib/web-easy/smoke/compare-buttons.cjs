...
const props = [
  // display is context-sensitive: flex items are blockified in computed style.
...
(async () => {
  ...
  await pageB.waitForSelector('#solar2-buttons', { timeout: 30000 });
  await pageB.waitForSelector('.we-button-row', { timeout: 30000 });
  await pageB.waitForFunction(() => {
    const html = document.documentElement;
    const sample = document.querySelector('#solar2-buttons')?.parentElement?.querySelector('.we-button-row .we-button');
    if (!html || !sample) return false;
    const hasTheme = html.classList.contains('we-theme-solar2');
    const bg = getComputedStyle(sample).backgroundColor || '';
    return hasTheme && bg !== 'rgb(239, 239, 239)';
  }, { timeout: 30000 });
  ...
  const b = await collect(pageB, `() => {
    const heading = document.querySelector('#solar2-buttons');
    if (!heading) {
      return { target: null, diag: 'web-easy buttons section missing' };
    }
    const grid = heading.nextElementSibling;
    if (!grid || !grid.classList.contains('showcase-buttons-grid')) {
      return { target: null, diag: 'web-easy buttons grid missing' };
    }
    const rows = Array.from(grid.querySelectorAll('.we-button-row'));
    const rowNode = ${JSON.stringify(row)} === 'sizes'
