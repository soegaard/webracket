...
if [ "${SMOKE_SKIP_COMPILE:-0}" = "1" ]; then
  echo "Skipping contract compile (SMOKE_SKIP_COMPILE=1)."
else
...
if ! command -v node >/dev/null 2>&1; then
  echo "node is required for headless contract runner."
  exit 2
fi
...
raco static-web --port "$PORT" --dir "$SCRIPT_DIR" >/tmp/web-easy-contract-headless.log 2>&1 &
SERVER_PID=$!
...
node "$SCRIPT_DIR/check-contract-headless.mjs"
