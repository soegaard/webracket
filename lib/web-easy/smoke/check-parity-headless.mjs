...
async function main() {
  ...
  const warnMs = Number.isFinite(parsedWarnMs) && parsedWarnMs > 0
    ? parsedWarnMs
    : 2000;
  const timingOut = process.env.SMOKE_TIMING_OUT || "/tmp/web-easy-parity-timings.tsv";
  ...
  try {
    console.log(`parity headless timeout: ${timeoutMs}ms`);
    const mark = guard.mark();
    await page.goto(`${baseUrl}/test-browser-parity-dashboard.html`, { waitUntil: "domcontentloaded" });
    ...
    guard.assertSince(mark, "Parity dashboard run");
    console.log(text);
    ...
    await browser.close();
    console.error("Headless parity smoke run failed.");
