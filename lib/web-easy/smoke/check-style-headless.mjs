...
async function runOne(page, guard, baseUrl, timeoutMs, testPage) {
  ...
  guard.assertSince(mark, `Style test run (${testPage})`);
...
async function main() {
  ...
  try {
    ...
    for (const testPage of tests) {
      const result = await runOne(page, guard, baseUrl, timeoutMs, testPage);
      results.push(result);
