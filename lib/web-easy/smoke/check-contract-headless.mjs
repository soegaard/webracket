...
async function main() {
  ...
  const regressionPct = Number.isFinite(Number(regressionPctRaw))
    ...
    : 150;
  const retryOnRegression = process.env.SMOKE_TIMING_RETRY_ON_REGRESSION !== "0";
  ...
  async function runDashboard(dashboardPath) {
    ...
    try {
      guard.assertSince(mark, `Contract dashboard run (${dashboardPath})`);
    } catch (err) {
      let currentPage = "";
      let lastCompletedPage = "";
      try {
        const pages = await page.evaluate(() => ({
          current: String(window.__smokeCurrentPage || ""),
          last: String(window.__smokeLastCompletedPage || ""),
        }));
        currentPage = pages.current;
        lastCompletedPage = pages.last;
      } catch (_ignored) {
        currentPage = "";
        lastCompletedPage = "";
      }
      const parts = [];
      if (currentPage) {
        parts.push(`while running ${currentPage}`);
      }
      if (lastCompletedPage) {
        parts.push(`last completed ${lastCompletedPage}`);
      }
  ...
  try {
    ...
    if (baseline.size > 0) {
      ...
      if (regressions.length > 0 && retryOnRegression) {
        console.log(`Timing regressions detected; re-running dashboards once to rule out flakes.`);
        const reruns = [];
        for (const dash of dashboards) {
          const run = await runDashboard(dash);
          console.log(`[recheck ${dash}] ${run.summary.text}`);
          reruns.push(run);
          if (!run.summary.ok) {
            break;
          }
        ...
        if (reruns.length === dashboards.length && reruns.every((r) => r.summary.ok)) {
          const rerowMap = new Map();
          for (const row of reruns.flatMap((r) => r.rows)) {
            if (row.durationMs >= 0) {
              rerowMap.set(row.page, row.durationMs);
            }
          }
          regressions = regressions.filter((r) => {
            const reMs = rerowMap.get(r.page);
            if (!Number.isFinite(reMs)) return true;
            return reMs > r.limit;
          });
        }
      }
    ...
    if (consoleErrors.length > 0) {
      ...
      for (const line of consoleErrors.slice(-12)) {
        console.error(`  - ${line}`);
      }
    }
    if (consoleLogs.length > 0) {
      console.error("Recent console logs:");
      for (const line of consoleLogs.slice(-16)) {
        console.error(`  - ${line}`);
      }
    }
    process.exit(1);
