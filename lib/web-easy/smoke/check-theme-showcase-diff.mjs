...
async function main() {
  ...
  try {
    ...
    const demoTop = await page.screenshot({ clip: { x: 0, y: 0, width: 1440, height: 360 } });
    ...
    const genTop = await page.screenshot({ clip: { x: 0, y: 0, width: 1440, height: 360 } });
    const genFull = await page.screenshot({ fullPage: true });

    const top = await comparePngBuffers(page, demoTop, genTop);
    const full = await comparePngBuffers(page, demoFull, genFull);
