...
if [ "${SMOKE_SKIP_COMPILE:-0}" = "1" ]; then
  echo "Skipping theme compile (SMOKE_SKIP_COMPILE=1)."
else
...
if ! command -v node >/dev/null 2>&1; then
  echo "node is required for headless theme runner."
  exit 2
fi
...
raco static-web --port "$PORT" --dir "$SCRIPT_DIR" >/tmp/web-easy-theme-headless.log 2>&1 &
SERVER_PID=$!
...
node "$SCRIPT_DIR/check-theme-headless.mjs"
