...
async function main() {
  ...
  const warnMs = Number.isFinite(parsedWarnMs) && parsedWarnMs > 0
    ? parsedWarnMs
    : 2000;
  const timingOut = process.env.SMOKE_TIMING_OUT || "/tmp/web-easy-theme-timings.tsv";
  ...
  try {
    console.log(`theme headless timeout: ${timeoutMs}ms`);
    const mark = guard.mark();
    await page.goto(`${baseUrl}/test-browser-theme-contract-dashboard.html`, { waitUntil: "domcontentloaded" });
    ...
    guard.assertSince(mark, "Theme dashboard run");
    console.log(text);
    ...
    await browser.close();
    console.error("Headless theme run failed.");
