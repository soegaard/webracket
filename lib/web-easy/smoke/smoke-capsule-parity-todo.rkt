...
;;
;; Exports:
;;   parity-todo-make-page      Build and mount the parity page under root.
;;   parity-todo-run-test       Execute capsule-local setup checks.
;;   parity-todo-cleanup        Destroy mounted renderer state for this capsule.

(define-values (parity-todo-make-page parity-todo-run-test parity-todo-cleanup)
  (let ()
    ;; Constants for parity-todo capsule state.
    (define parity-todo-renderer #f) ; Mounted renderer for this capsule.

    ;; parity-todo-make-page : any/c -> void?
    ;;   Build and mount the parity page under root.
    (define (parity-todo-make-page root)
      ;;;
      ...
      (set! parity-todo-renderer app-renderer)
      (mount-renderer! app-renderer root)
      (void))

    ;; parity-todo-run-test : any/c -> boolean?
    ;;   Return #t when capsule setup completed.
    (define (parity-todo-run-test _root)
      (and parity-todo-renderer #t))

    ;; parity-todo-cleanup : any/c -> void?
    ;;   Destroy the mounted renderer for this capsule.
    (define (parity-todo-cleanup _root)
      (when parity-todo-renderer
        (renderer-destroy parity-todo-renderer)
        (set! parity-todo-renderer #f))
      (void))

    (values parity-todo-make-page parity-todo-run-test parity-todo-cleanup)))
