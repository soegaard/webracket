...
if [ "${SMOKE_SKIP_COMPILE:-0}" = "1" ]; then
  echo "Skipping style compile (SMOKE_SKIP_COMPILE=1)."
else
...
if ! command -v node >/dev/null 2>&1; then
  echo "node is required for headless style runner."
  exit 2
fi
...
raco static-web --port "$PORT" --dir "$SCRIPT_DIR" >/tmp/web-easy-style-headless.log 2>&1 &
SERVER_PID=$!
...
SMOKE_WARN_MS_STYLE="${SMOKE_WARN_MS_STYLE:-${SMOKE_WARN_MS:-2000}}" \
node "$SCRIPT_DIR/check-style-headless.mjs"
