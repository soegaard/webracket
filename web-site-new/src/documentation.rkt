  ...
  `(div (@ (class "page page--docs page--glance"))
        ...
        (section (@ (class "docs-hero"))
                 (div (@ (class "hero-panel"))
                      ...
                      (p (@ (class "hero-sublead"))
                         (a (@ (href "documentation.html")) "Documentation")
                         " / Extended Example: JSXGraph board and points")
                      (h1 (@ (class "hero-title")) "Extended Example: JSXGraph Board and Points")
                      (p (@ (class "hero-lead"))
                         "End-to-end setup of a JSXGraph board, followed by point creation "
                         "and board updates using the WebRacket bindings.")))
        ,(section-block
          "Extended Example: JSXGraph Board and Points"
          #f
          (list
  ...
  `(div (@ (class "page page--docs page--glance"))
        ...
        (section (@ (class "docs-hero"))
                 (div (@ (class "hero-panel"))
                      ...
                      (p (@ (class "hero-sublead"))
                         (a (@ (href "documentation.html")) "Documentation")
                         " / Extended Example: JSXGraph geometry constructors")
                      (h1 (@ (class "hero-title"))
                          "Extended Example: JSXGraph Geometry Constructors")
                      (p (@ (class "hero-lead"))
...
(define (installation-page)
  `(div (@ (class "page"))
        ...
        ,(section-block
          "Prerequisites"
          "Short checklist for a working WebRacket setup."
          (list
           (make-ul-list
            (list
             "wasm-tools (Bytecode Alliance) v1.243.0+"
             "Node.js with support for --experimental-wasm-exnref"
             "Racket 9.0 or newer"
             "raco-static-web"
             "A local clone of the WebRacket repository")))
          #f
          #f)
        ,(section-block
          "Installation Steps"
          ...
          (list
           (card-grid
            (list
             (list `(h3 "1. wasm-tools")
                   `(p "Download the latest wasm-tools release from "
                       (a (@ (href "https://github.com/bytecodealliance/wasm-tools/releases")
                             (target "_blank")
                             (rel "noreferrer noopener"))
                          "GitHub")
                       ".")
                   `(p "Extract and add it to your PATH.")
                   ...
                   (callout
                     'note
                     "Troubleshooting"
                     `(p "On macOS, you may get a Privacy & Security prompt. "
                         "Allow " (code "wasm-tools") " to run from System Settings.")
             ...
             (list `(h3 "2. Node.js")
                   ...
                   (callout
                     'note
                     "Troubleshooting"
                     `(p "WebRacket needs support for "
                         (code "--experimental-wasm-exnref")
                         ", so the command above must start without errors.")))
             (list `(h3 "3. Racket")
                   `(p "Install Racket 9 or newer from "
                       (a (@ (href "https://download.racket-lang.org/")
                             (target "_blank")
                             (rel "noreferrer noopener"))
                          "download.racket-lang.org")
                       "."))
             (list `(h3 "4. raco-static-web")
                   `(p "Install the local web server package.")
                   `(pre (code "raco pkg install raco-static-web"))
                   `(p "Run it in a folder with an HTML file:")
                   `(pre (code "raco static-web")))
             (list `(h3 "5. nanopass")
                   `(p "Install " (code "nanopass") ".")
                   `(pre (code "raco pkg install nanopass")))             
             (list `(h3 "6. Clone the WebRacket repo")
                   `(p "Clone the WebRacket repository (compiler and examples).")
                   `(pre (code "git clone https://github.com/soegaard/webracket.git")))
             (list `(h3 "7. Make WebRacket a local package")
             ...
             (list `(h3 "8. Quick test: run the examples")
