  ...
  (definitions
    ...
    (define (obviously-single-valued-expression? e)
      (nanopass-case (LFE Expr) e
        ...
        [else
         #f]))
    ;; begin-prefix+tail : LFE Expr LFE Expr* -> values (listof LFE Expr) LFE Expr
    ;;   Split a non-empty begin-style sequence into prefix and tail.
    (define (begin-prefix+tail e0 e1)
      (define es (cons e0 e1))
      (values (drop-right es 1)
              (last es)))
    ;; obviously-number-valued-expression? : LFE Expr -> boolean?
    ;;   Recognize expressions that obviously produce exactly one numeric value.
    (define (obviously-number-valued-expression? e)
      (define (numeric-primitive? e0)
        (and (variable? e0)
             (memq (syntax-e (variable-id e0))
                   '(+ - * / add1 sub1 abs))))
      (nanopass-case (LFE Expr) e
        [(quote ,s ,d)
         (number? (datum-value d))]
        [(if ,s ,e0 ,e1 ,e2)
         (and (obviously-single-valued-expression? e0)
              (obviously-number-valued-expression? e1)
              (obviously-number-valued-expression? e2))]
        [(begin ,s ,e0 ,e1 ...)
         (let-values ([(prefix tail) (begin-prefix+tail e0 e1)])
           (and (andmap obviously-single-valued-expression? prefix)
                (obviously-number-valued-expression? tail)))]
        [(begin0 ,s ,e0 ,e1 ...)
         ...
         (and (andmap (λ (xs) (= (length xs) 1)) x*)
              ...
              (let-values ([(prefix tail) (begin-prefix+tail e0 e1)])
                (and (andmap obviously-single-valued-expression? prefix)
                     (obviously-number-valued-expression? tail))))]
        [(app ,s ,e0 ,e1 ...)
         (and (numeric-primitive? e0)
              (andmap obviously-number-valued-expression? e1))]
        [else
         #f]))
    ;; obviously-exact-integer-valued-expression? : LFE Expr -> boolean?
    ;;   Recognize expressions that obviously produce exactly one exact integer value.
    (define (obviously-exact-integer-valued-expression? e)
      (define (exact-integer-primitive? e0)
        (and (variable? e0)
             (memq (syntax-e (variable-id e0))
                   '(bitwise-and bitwise-ior bitwise-xor add1 sub1))))
      (nanopass-case (LFE Expr) e
        [(quote ,s ,d)
         (exact-integer? (datum-value d))]
        [(if ,s ,e0 ,e1 ,e2)
         (and (obviously-single-valued-expression? e0)
              (obviously-exact-integer-valued-expression? e1)
              (obviously-exact-integer-valued-expression? e2))]
        [(begin ,s ,e0 ,e1 ...)
         (let-values ([(prefix tail) (begin-prefix+tail e0 e1)])
           (and (andmap obviously-single-valued-expression? prefix)
                (obviously-exact-integer-valued-expression? tail)))]
        [(begin0 ,s ,e0 ,e1 ...)
         (and (obviously-exact-integer-valued-expression? e0)
              (andmap obviously-single-valued-expression? e1))]
        [(let-values ,s ([(,x* ...) ,e*] ...) ,e0 ,e1 ...)
         (and (andmap (λ (xs) (= (length xs) 1)) x*)
              (andmap obviously-single-valued-expression? e*)
              (let-values ([(prefix tail) (begin-prefix+tail e0 e1)])
                (and (andmap obviously-single-valued-expression? prefix)
                     (obviously-exact-integer-valued-expression? tail))))]
        [(app ,s ,e0 ,e1 ...)
         (and (exact-integer-primitive? e0)
              (andmap obviously-exact-integer-valued-expression? e1))]
        [else
         #f]))
    ...
    (define (quoted-list-element e i)
      (nanopass-case (LFE Expr) e
        ...
        [else
         no-partial-fold]))
    ;; quoted-list-tail : LFE Expr exact-nonnegative-integer? -> any/c
    ;;   Return the tail after `i` cdr steps of a quoted pair/list, or the no-partial-fold sentinel.
    (define (quoted-list-tail e i)
      (nanopass-case (LFE Expr) e
        [(quote ,s ,d)
         (define v (datum-value d))
         (with-handlers ([exn:fail? (λ (_exn) no-partial-fold)])
           (list-tail v i))]
        [else
         no-partial-fold]))
    ...
    (define (annihilate-on-exact-zero s e* valued?)
      ...
      (and (< (length kept) (length e*))
           (sequence-then-quote s kept 0 valued?)))
    ;; fold-exact-integer-subset : syntax? variable? (listof LFE Expr) (exact-integer? exact-integer? -> exact-integer?) any/c (LFE Expr -> boolean?)
    ;;                          -> (or/c LFE Expr #f)
    ;;   Combine exact integer constants inside an n-ary primitive application.
    (define (fold-exact-integer-subset s x e* op ident valued?)
    ...
    (define (partial-fold-primitive-application s x e*)
      ...
      (match (list sym e*)
        ...
        [(list 'caar (list e0))
         ; (caar '((a . b) . d)) => 'a
         ; (caar (cons (cons a b) d)) => a
         ; (caar (list (cons a b) ...)) => a
         ; (caar (list* (cons a b) ...)) => a
         (nested-pair-access-result s e0 '() 'car)]
        [(list 'cdr (list e0))
         ; (cdr '(a . d)) => 'd
         ; (cdr (cons x y)) => y
         ; (cdr (list x ...)) => (list ...)
        ...
        [(list 'cdar (list e0))
         ; (cdar '((a . b) . d)) => 'b
         ; (cdar (cons (cons a b) d)) => b
         ; (cdar (list (cons a b) ...)) => b
         ; (cdar (list* (cons a b) ...)) => b
         (nested-pair-access-result s e0 '() 'cdr)]
        [(list 'cadr (list e0))
         ; (cadr '(a b ...)) => 'b
         ; (cadr (list a b ...)) => b
         ; (cadr (list* a b c ...)) => b
        ...
        [(list 'cddr (list e0))
         ; (cddr '(a b . d)) => 'd
         ; (cddr (list a b ...)) => (list ...)
         ; (cddr (list* a b c ...)) => (list* c ...)
         (or (quote-when-found s (quoted-list-tail e0 2))
             (match (primitive-application-arguments e0 'list)
               [#f #f]
               [es (and (>= (length es) 2)
                        (constructor-list-tail-result s es 2))])
             (match (primitive-application-arguments e0 'list*)
               [#f #f]
               [es (and (>= (length es) 3)
                        (constructor-list*-tail-result s es 2))]))]
        [(list 'cadddr (list e0))
         ; (cadddr '(a b c d ...)) => 'd
         ; (cadddr (list a b c d ...)) => d
         ; (cadddr (list* a b c d e ...)) => d
        ...
        [(list 'cdddr (list e0))
         ; (cdddr '(a b c . d)) => 'd
         ; (cdddr (list a b c ...)) => (list ...)
         ; (cdddr (list* a b c d ...)) => (list* d ...)
         (or (quote-when-found s (quoted-list-tail e0 3))
             (match (primitive-application-arguments e0 'list)
               [#f #f]
               [es (and (>= (length es) 3)
        ...
        [(list 'bitwise-ior _)
         ...
         (or (and (= (length e*) 1)
                  (keep-valued obviously-exact-integer-valued-expression? (car e*)))
             (fold-exact-integer-subset s x e* bitwise-ior 0 obviously-exact-integer-valued-expression?)
        ...
        [(list 'bitwise-xor _)
         ...
         (or (and (= (length e*) 1)
                  (keep-valued obviously-exact-integer-valued-expression? (car e*)))
             (fold-exact-integer-subset s x e* bitwise-xor 0 obviously-exact-integer-valued-expression?)
             (drop-identity-arguments s x e* quoted-zero? 0 obviously-exact-integer-valued-expression?))]
        [(list 'bitwise-and _)
         ; (bitwise-and x) => x
...
(module+ test
  (let ([test (λ (stx)
    ...
    (check-equal? (test #'(cdr '(1 . 2)))
                  ''2)
    (check-equal? (test #'(caar '((1 . 2) . 3)))
                  ''1)
    (check-equal? (test #'(cdar '((1 . 2) . 3)))
                  ''2)
    ...
    (check-equal? (test #'(cdr (cons 1 2)))
                  '(let-values (((pf0) '1))
                     (let-values (((pf1) '2))
                       pf1)))
    (check-equal? (test #'(caar (cons (cons 1 2) 3)))
                  '(let-values (((pf0) '1))
                     (let-values (((pf1) '2))
                       (let-values (((pf2) '3))
                         pf0))))
    (check-equal? (test #'(cdar (cons (cons 1 2) 3)))
                  '(let-values (((pf0) '1))
                     (let-values (((pf1) '2))
                       (let-values (((pf2) '3))
                         pf1))))
    ...
    (check-equal? (test #'(cadr (list 1 2 3)))
                  '(let-values (((pf0) '1))
                     (let-values (((pf1) '2))
                       (let-values (((pf2) '3))
                         pf1))))
    (check-equal? (test #'(caar (list (cons 1 2) 3 4)))
                  '(let-values (((pf0) '1))
                     (let-values (((pf1) '2))
                       (let-values (((pf2) '3))
                         (let-values (((pf3) '4))
                           pf0)))))
    (check-equal? (test #'(cdar (list (cons 1 2) 3 4)))
                  '(let-values (((pf0) '1))
                     (let-values (((pf1) '2))
                       (let-values (((pf2) '3))
                         (let-values (((pf3) '4))
                           pf1)))))
    (check-equal? (test #'(caddr '(1 2 3 4)))
                  ''3)
    (check-equal? (test #'(caddr (list 1 2 3 4)))
                  '(let-values (((pf0) '1))
                     (let-values (((pf1) '2))
                       (let-values (((pf2) '3))
                         (let-values (((pf3) '4))
                           pf2)))))
    (check-equal? (test #'(cddr '(1 2 3 4)))
                  ''(3 4))
    (check-equal? (test #'(cddr '(1 2 . 3)))
                  ''3)
    (check-equal? (test #'(cddr (list 1 2 3 4)))
                  '(let-values (((pf0) '1))
                     (let-values (((pf1) '2))
                       (let-values (((pf2) '3))
                         (let-values (((pf3) '4))
                           (list pf2 pf3))))))
    (check-equal? (test #'(cadddr '(1 2 3 4 5)))
                  ''4)
    (check-equal? (test #'(cdddr '(1 2 3 4 5)))
                  ''(4 5))
    (check-equal? (test #'(cdddr '(1 2 3 . 4)))
                  ''4)
    (check-equal? (test #'(cadddr (list 1 2 3 4 5)))
                  '(let-values (((pf0) '1))
                     (let-values (((pf1) '2))
                       (let-values (((pf2) '3))
                         (let-values (((pf3) '4))
                           (let-values (((pf4) '5))
                             pf3))))))
    (check-equal? (test #'(cdddr (list 1 2 3 4 5)))
                  '(let-values (((pf0) '1))
                     (let-values (((pf1) '2))
                       (let-values (((pf2) '3))
                         (let-values (((pf3) '4))
    ...
    (check-equal? (test #'(values x))
                  '(#%top . x))
    (check-equal? (test #'(values (values 1 2)))
                  '(values (values '1 '2)))
    ...
    (check-equal? (test #'(bitwise-ior (if b 1 2)))
                  '(if (#%top . b) '1 '2))
    (check-equal? (test #'(bitwise-ior x))
                  '(bitwise-ior (#%top . x)))
    (check-equal? (test #'(bitwise-ior 1 2 x 4))
                  '(bitwise-ior '7 (#%top . x)))
    (check-equal? (test #'(bitwise-xor 1 2 x 3))
                  '(bitwise-xor '1 '2 (#%top . x) '3))
    (check-equal? (test #'(bitwise-xor (if b 1 2)))
                  '(if (#%top . b) '1 '2))
    (check-equal? (test #'(bitwise-and -1 x -1))
                  '(bitwise-and '-1 (#%top . x) '-1))
    (check-equal? (test #'(bitwise-and (if b 1 2)))
                  '(if (#%top . b) '1 '2))
    (check-equal? (test #'(bitwise-and x 0 y))
                  '(bitwise-and (#%top . x) '0 (#%top . y)))
    (check-equal? (test #'(bitwise-and (add1 x) 0))
                  '(bitwise-and (add1 (#%top . x)) '0))
    ...
    (check-equal? (test #'(string? "hello"))
                  ''#t)
    (check-equal? (test #'(string=? "a" "a"))
                  ''#t)
    (check-equal? (test #'(zero? 0))
                  ''#t)
...
(define-pass lower-letrec-values : LFE2+ (T) -> LFE2+ ()
  (definitions
    ...
    (define (prepare-reclassified-scc-plan-only s component body-referenced)
      (define classified+refs-rev
        (for/fold ([classified+refs-rev '()])
                  ([binding+facts (in-list (letrec-scc-component-binding+facts component))])
          (define binding
          ...
          (define local-clause
            (if (and (eq? default-kind 'unreferenced)
                ...
                (list default-kind xs rhs)))
          (cons (cons local-clause all-refs) classified+refs-rev)))
      (prepare-classified+refs-plan-only s (reverse classified+refs-rev)))
    ...
    (define (prepare-single-used-scc-plan-only s component used-xs)
      (define classified+refs-rev
        (for/fold ([classified+refs-rev '()])
                  ([binding+facts (in-list (letrec-scc-component-binding+facts component))])
          (define binding
      ...
      (define components
        (for/list ([component (in-list (letrec-scc-components classified))])
          ...
          (define needs-default-plan?
            (or (not singleton-component?)
                (letrec-scc-binding-self-recursive?
                 (first (letrec-scc-component-bindings component)))))
          ...
          (define single-used-classified
            (and singleton-component?
                 ...
                 (list (caar (reverse single-used-classified+refs*)))))
          (define single-used-classified+refs**
            (and single-used-classified+refs*
                 (reverse single-used-classified+refs*)))
          (define default-plan
            (and needs-default-plan?
                 (prepare-classified+refs-plan-only s default-classified+refs)))
          (define single-used-plan
            (and needs-default-plan?
                 single-used-classified+refs**
                 (prepare-classified+refs-plan-only s single-used-classified+refs**)))
          (struct-copy letrec-scc-component component
        ...
        (define next-body
          (match sorted-scc
            [(list binding)
             ...
             (define dead-pure-singleton?
               (letrec-scc-binding-facts-dead-pure-singleton? facts))
             (define-values (local-kind local-plan)
               (cond
                 [(not body-uses-scc?)
                  (values (letrec-scc-binding-facts-default-kind facts)
             ...
             (define local-plan
               (cond
                 ...
                 [(and (= used-unreferenced-count 1)
                  ...
                  (letrec-scc-component-single-used-plan metadata)]
                 [(= used-unreferenced-count 1)
                  (prepare-single-used-scc-plan-only s
                                                     metadata
                                                     used-unreferenced-xs)]
                 [else
                  (prepare-reclassified-scc-plan-only s
                                                      metadata
                                                      body-referenced)]))
             (cond
               [(and (not body-uses-scc?)
