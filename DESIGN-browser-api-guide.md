...
The generic bridge documentation should be explicit about the JS-to-WebRacket value conversion.

When documenting `value` results, be precise about:
- recursive conversion
- `null` and `undefined` handling
- `void`
- `external`
- what happens to arrays, bytes, wrapper objects, and other special cases

The important caveat from the bridge work is that recursive encoding exists, so cyclic data structures are not supported and may overflow the stack.

If a value cannot be converted, say so directly.
