       ...
       ; If the hash code is 0 it hash hasn't been computed yet.
        ...
        ;; https://docs.racket-lang.org/reference/bytestrings.html
         ...
         (func $real->floating-point-bytes (type $Prim5)
               ...
               (param $start-raw (ref eq))  ;; exact-nonnegative-integer?, defaults to 0
               (result           (ref eq))
               ...
               ;; ===============================================================
               ;; Phase 1: VALIDATION ONLY (no lasting extraction yet)
               ;; ===============================================================

               ;; --- Validate $size-raw is a fixnum 4 or 8 ---
               (if (i32.eqz (ref.test (ref i31) (local.get $size-raw)))
                   (then (call $raise-check-fixnum (local.get $size-raw)) (unreachable)))
               (local.set $size/tag (i31.get_u (ref.cast (ref i31) (local.get $size-raw))))
               (if (i32.and (local.get $size/tag) (i32.const 1))
                   (then (call $raise-check-fixnum (local.get $size-raw)) (unreachable)))
               (local.set $size (i32.shr_u (local.get $size/tag) (i32.const 1)))
               ...
               ;; --- Validate $start-raw (if present) is exact-nonnegative-integer? ---
               (if (ref.eq (local.get $start-raw) (global.get $missing))
                   ...
                   (else
                    (if (i32.eqz (ref.test (ref i31) (local.get $start-raw)))
                        ...
                        (else))
                    (local.set $start/tag
                               (i31.get_s (ref.cast (ref i31) (local.get $start-raw))))
                    (if (i32.and (local.get $start/tag) (i32.const 1))
                        (then (call $raise-check-fixnum (local.get $start-raw))
                              (unreachable))
                        (else))
                    (local.set $start
                               (i32.shr_s (local.get $start/tag) (i32.const 1)))
                    (if (i32.lt_s (local.get $start) (i32.const 0))
                        (then (call $raise-argument-error (local.get $start-raw))
                              (unreachable)))))

               ;; --- Validate $dest-raw branch contracts (type, mutability, capacity) ---
               (if (ref.eq (local.get $dest-raw) (global.get $missing))
                   (then
                    ;; If dest is omitted, start must be omitted, too.
                    (if (i32.eqz (ref.eq (local.get $start-raw) (global.get $missing)))
                        (then (call $raise-argument-error (local.get $start-raw))
                              (unreachable))
                        (else)))
                   (else
                    ;; Present: must be bytes?, mutable, and large enough.
                    (if (i32.eqz (ref.test (ref $Bytes) (local.get $dest-raw)))
                        (then (call $raise-check-bytes (local.get $dest-raw)) (unreachable)))
                    (local.set $dest*
                               (ref.cast (ref $Bytes) (local.get $dest-raw)))
                    (if (i32.eq (struct.get $Bytes $immutable (local.get $dest*))
                                (i32.const 1))
                        (then (call $raise-expected-mutable-bytes (local.get $dest-raw))
                              (unreachable))
                        (else))
                    ;; If start was omitted, treat it as 0 for capacity check only.
                    (local.set $start (i32.const 0))
                    (if (ref.eq (local.get $start-raw) (global.get $missing))
                        (then)
                        (else
                         (let ((local $start/tag2 i32))
                           (local.set $start/tag2
                                      (i31.get_s (ref.cast (ref i31) (local.get $start-raw))))
                           (local.set $start
                                      (i32.shr_s (local.get $start/tag2) (i32.const 1)))))
                        (if (i32.lt_u
                             (call $i8array-length
                                   (struct.get $Bytes $bs (local.get $dest*)))
                             (i32.add (local.get $start) (local.get $size)))
                            (then (call $raise-argument-error (local.get $dest-raw))
                                  (unreachable))))))

               ;; --- Validate $x is a number (flonum or fixnum) ---
               (if (i32.or (ref.test (ref $Flonum) (local.get $x))
                           (ref.test (ref i31) (local.get $x)))
                   (then)
                   (else (call $raise-expected-number (local.get $x)) (unreachable)))

               ;; ===============================================================
               ;; Phase 2: EXTRACTION / INITIALIZATION (straight-line, definite)
               ;; ===============================================================
               ...
               ;; big-endian? flag (defaults to system-endian)
               ...
               ;; destination + underlying array
               (local.set $arr (array.new $I8Array ,(Imm 0) ,(Imm 0)))
               (if (ref.eq (local.get $dest-raw) (global.get $missing))
                   (then
                    (local.set $dest
                               (ref.cast (ref $Bytes)
               ...
               (if (ref.test (ref $Flonum) (local.get $x))
                   ...
                   (else
                    (if (ref.test (ref i31) (local.get $x))
                        ...
                        (else ;; unreachable by validation, but keeps structure explicit
                         (call $raise-expected-number (local.get $x)) (unreachable)))))
               ...
               (if (i32.eq (local.get $size) (i32.const 8))
                   (then
                    ...
                    (if (i32.eqz (local.get $big))
                        (then
                         ...
                         (call $i8array-set! (local.get $arr)
                         ...
                         (call $i8array-set! (local.get $arr)
                               (i32.add (local.get $start) (i32.const 6))
                               (i32.wrap_i64 (i64.shr_u (local.get $bits64) (i64.const 48))))
                         (call $i8array-set! (local.get $arr)
                               (i32.add (local.get $start) (i32.const 7))
                   ...
                   (else
                    (local.set $bits32
                               (i32.reinterpret_f32 (f32.demote_f64 (local.get $val))))
                    (if (i32.eqz (local.get $big))
                        (then
                         (call $i8array-set! (local.get $arr) (local.get $start)
                         ...
                         (call $i8array-set! (local.get $arr)
                               (i32.add (local.get $start) (i32.const 1))
                               (i32.and (i32.shr_u (local.get $bits32) (i32.const 8))
                                        (i32.const 255)))
                         (call $i8array-set! (local.get $arr)
                               (i32.add (local.get $start) (i32.const 2))
                               (i32.and (i32.shr_u (local.get $bits32) (i32.const 16))
                                        (i32.const 255)))
                         (call $i8array-set! (local.get $arr)
                               (i32.add (local.get $start) (i32.const 3))
                               (i32.and (i32.shr_u (local.get $bits32) (i32.const 24))
                                        (i32.const 255))))
                        (else
                         (call $i8array-set! (local.get $arr) (local.get $start)
                               (i32.and (i32.shr_u (local.get $bits32) (i32.const 24))
